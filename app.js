const LS_KEY='workdays_v1',CFG_KEY='workcfg_v5_2',OUTBOX_KEY='work_outbox_v1';
...
function minutesFromHHMM(h){const [a,b]=h.split(':').map(Number);return a*60+b;} function hhmmFromMinutes(m){const h=Math.floor(m/60),n=m%60;return`${String(h).padStart(2,'0')}:${String(n).padStart(2,'0')}`;}
function nowHHMM(){const d=new Date();return`${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;}
function parseLS(){try{return JSON.parse(localStorage.getItem(LS_KEY))||{}}catch{return{}}} function saveLS(o){localStorage.setItem(LS_KEY,JSON.stringify(o))}
function parseCFG(){try{return JSON.parse(localStorage.getItem(CFG_KEY))||{}}catch{return{}}} function saveCFG(c){localStorage.setItem(CFG_KEY,JSON.stringify(c))}
function parseOutbox(){try{return JSON.parse(localStorage.getItem(OUTBOX_KEY))||[]}catch{return[]}} function saveOutbox(a){localStorage.setItem(OUTBOX_KEY,JSON.stringify(a))}
...
function daysInMonthCount(y,m){return new Date(y,m,0).getDate()} function weekdayName(d){return['Dom','Lun','Mar','Mer','Gio','Ven','Sab'][d]}
...
/* Picker */
const picker={field:null,date:null};function buildPickerOptions(){const h=$('#pickHour'),m=$('#pickMin');if(h.options.length===0){for(let i=0;i<24;i++){const o=document.createElement('option');o.value=String(i).padStart(2,'0');o.textContent=o.value;h.appendChild(o)}}if(m.options.length===0){for(let i=0;i<60;i++){const o=document.createElement('option');o.value=String(i).padStart(2,'0');o.textContent=o.value;m.appendChild(o)}}}
function openPicker(dateISO,field,preset){picker.field=field;picker.date=dateISO;buildPickerOptions();const[ph,pm]=(preset||'08:00').split(':');$('#pickHour').value=ph;$('#pickMin').value=pm;$('#modalPicker').classList.add('show');$('#modalPicker').setAttribute('aria-hidden','false')}
...
/* Recenti */
let cloudCache=null;async function fetchCloudLast(days=90){const cfg=parseCFG();if(!cfg.syncEnabled||!cfg.syncUrl)return null;const r=await fetch(cfg.syncUrl+`?op=last&days=${days}`,{headers:{'X-Auth':cfg.syncToken||''}});if(!r.ok)throw new Error('HTTP '+r.status);const arr=await r.json();const map={};if(Array.isArray(arr)){for(const a of arr){map[a.date]={start:a.start||null,breakStart:a.breakStart||null,breakEnd:a.breakEnd||null,end:a.end||null,finalized:!!a.finalized}}}return map}
async function updateRecentiUI(){const list=$('#recentiList');list.innerHTML='';const d=new Date();const y=d.getFullYear(),m=d.getMonth()+1;const totalDays=daysInMonthCount(y,m);const cfg=parseCFG();const banner=$('#recentiBanner');const needOnline=!(cfg.syncEnabled&&cfg.syncUrl);banner.hidden=!needOnline;if(needOnline&&!navigator.onLine){const p=document.createElement('p');p.className='muted';p.textContent='Offline. Connettiti per caricare orari recenti dal cloud.';list.appendChild(p);return}try{cloudCache=await fetchCloudLast(90)}catch(e){console.warn('Cloud fetch error',e);cloudCache=null}const local=parseLS();for(let i=1;i<=totalDays;i++){const iso=`${y}-${String(m).padStart(2,'0')}-${String(i).padStart(2,'0')}`;const wd=new Date(iso+'T00:00:00').getDay();const weekend=(wd===0||wd===6);const rec=(cloudCache&&cloudCache[iso])||local[iso]||{};const extra=computeExtra(rec);const extraStr=(extra===null)?'—':`${extra>=0?'+':'−'}${hhmmFromMinutes(Math.abs(extra))}`;const el=document.createElement('div');el.className='item'+(weekend?' weekend':'');el.innerHTML=`<div class="muted">${iso} ${weekdayName(wd)}</div><div class="extra">${extraStr}</div><button class="editbtn" data-date="${iso}">Modifica</button>`;el.addEventListener('click',ev=>{if(ev.target&&ev.target.classList.contains('editbtn'))return;openDayDetail(iso)});el.querySelector('.editbtn').addEventListener('click',ev=>{ev.stopPropagation();openDayDetail(iso)});list.appendChild(el)}}
...
/* Totale mensile */
function ymCurrent(){const d=new Date();return`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`} function sumMonthLocal(ym){const data=parseLS();let tot=0;for(const k of Object.keys(data)){if(k.startsWith(ym)){const e=computeExtra(data[k]);if(e!==null)tot+=e}}return tot}
async function monthTotalFromCloud(ym){const cfg=parseCFG();if(!cfg.syncEnabled||!cfg.syncUrl)return null;const r=await fetch(`${cfg.syncUrl}?op=month&ym=${encodeURIComponent(ym)}`,{headers:{'X-Auth':cfg.syncToken||''}});if(!r.ok)throw new Error('HTTP '+r.status);const js=await r.json();return(js&&typeof js.totalExtraMinutes==='number')?js.totalExtraMinutes:null}
async function updateTotaleUI(){const ym=ymCurrent();const cfg=parseCFG();const banner=$('#totaleBanner');const needOnline=!(cfg.syncEnabled&&cfg.syncUrl);banner.hidden=!needOnline;let total=null;if(cfg.syncEnabled&&cfg.syncUrl&&navigator.onLine){try{total=await monthTotalFromCloud(ym)}catch(e){}}if(total===null)total=sumMonthLocal(ym);const s=total>=0?'+':'−';$('#bigTotal').textContent=`${s}${hhmmFromMinutes(Math.abs(total))}`}
...
async function trySync(){const cfg=parseCFG();if(!cfg.syncEnabled||!cfg.syncUrl)return;if(!navigator.onLine)return;const out=parseOutbox();if(out.length===0)return;try{let ok=0;for(const it of out){const r=await fetch(cfg.syncUrl,{method:'POST',headers:{'Content-Type':'application/json','X-Auth':cfg.syncToken||''},body:JSON.stringify({op:'upsert',payload:it})});if(!r.ok)throw new Error('HTTP '+r.status);const js=await r.json();if(js&&js.status==='ok')ok++}if(ok===out.length){saveOutbox([]);toast('Sync completata')}}catch(e){console.warn('Sync error',e)}}
...
function eagerSetup(){if(!localStorage.getItem(CFG_KEY)){saveCFG({startFixed:'08:00',defBreakStart:'12:30',defBreakEnd:'14:00',defEnd:'17:30',syncEnabled:false})}finalizePastDays();ensureDay(todayLocalISO());updateOggiUI();updateTotaleUI()}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // Picker
  $$('#rowsOggi .btn.ghost').forEach(btn=>{btn.addEventListener('click',()=>{const f=btn.getAttribute('data-edit');const t=todayLocalISO();const d=parseLS()[t]||{};const cur=(d[f]&&d[f].includes(':'))?d[f]:(f==='start'?(parseCFG().startFixed||'08:00'):'12:00');openPicker(t,f,cur)})});
  $('#btnPickerOk').addEventListener('click',applyPicker);$('#btnPickerCancel').addEventListener('click',closePicker);
...
function buildMonthOptions(){const sel=$('#pickMonth');sel.innerHTML='';const now=new Date();const months=['Gennaio','Febbraio','Marzo','Aprile','Maggio','Giugno','Luglio','Agosto','Settembre','Ottobre','Novembre','Dicembre'];for(let i=1;i<=12;i++){const dt=new Date(now.getFullYear(),now.getMonth()-i,1);const ym=`${dt.getFullYear()}-${String(dt.getMonth()+1).padStart(2,'0')}`;const op=document.createElement('option');op.value=ym;op.textContent=`${months[dt.getMonth()]} ${dt.getFullYear()}`;sel.appendChild(op)}sel.selectedIndex=0}
...
async function confirmMonthPicker(){const ym=$('#pickMonth').value;closeMonthPicker();let total=null;const cfg=parseCFG();if(cfg.syncEnabled&&cfg.syncUrl&&navigator.onLine){try{const r=await fetch(`${cfg.syncUrl}?op=month&ym=${encodeURIComponent(ym)}`,{headers:{'X-Auth':cfg.syncToken||''}});if(r.ok){const js=await r.json();if(js&&typeof js.totalExtraMinutes==='number')total=js.totalExtraMinutes}}catch(e){}}if(total===null)total=sumMonthLocal(ym);$('#monthResultTitle').textContent=`Totale ${ym}`;const s=total>=0?'+':'−';$('#monthResultValue').textContent=`${s}${hhmmFromMinutes(Math.abs(total))}`;$('#modalMonthResult').classList.add('show');$('#modalMonthResult').setAttribute('aria-hidden','false')}
